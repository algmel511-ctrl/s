// ====================================================================
// RavFen Shadow v7.0 - PUBG Mobile 4.5.0 iOS Tweak (Final-Fixed)
...
static void InitOffsets(void) {
    ...
    dispatch_once(&once, ^{
        ...
        gOffsets[OFF_VMC] = 0x7F8B000; // العنوان المباشر لمصفوفة العرض
...
// ====================================================================
// 👤 IsPlayer (مع إمكانية تجاوز الفحص للتجربة)
// ====================================================================
static BOOL gDebugBypassPlayerCheck = NO; // ضعها YES لتجاوز فحص الاسماء

static BOOL IsPlayer(uint64_t actor) {
    if (!actor) return NO;
    
    if (gDebugBypassPlayerCheck) {
        // للتجربة: تحقق فقط من وجود الـ Mesh والـ Health
        if (ReadFloat(actor + OFF(OFF_HP)) > 0.0f && ReadPtr(actor + OFF(OFF_MS))) {
            return YES;
        }
        return NO;
    }
...
// ====================================================================
// 🌐 WorldToScreen (تستخدم g_ViewMatrix)
// ====================================================================
...
static NSArray *GetPlayers(void) {
    ...
    // ✅ الإصلاح: قراءة ViewMatrix مباشرة دون استخدام ReadPtr
    ...
    for (int i = 0; i < cnt; i++) {
        @autoreleasepool {
            ...
            p.address = actor; p.x = ax; p.y = ay; p.z = az; p.health = hp; p.distance = dist;
            p.isAlive = (hp > 0.1f); p.teamId = team; p.isEnemy = (team != localTeam) || (team == 0);
...
// ====================================================================
// 🛡️ Lobby - تم تعطيل الفحص مؤقتاً للتجربة
// ====================================================================
static BOOL isInLobby = NO; // ✅ تم تعطيل فحص اللوبي

static void UpdateLobbyStatus(void) {
    // تعطيل فحص اللوبي للتأكد من عمل الـ Aimbot والـ ESP
    // يمكن إعادة تفعيله بعد التأكد من الثبات
    isInLobby = NO;
}
...
static void DoAimbot(NSArray *players) {
    ...
    BOOL aim = NO;
    switch (mode) { case AimbotMode_Lock: aim = YES; break; case AimbotMode_Fire: aim = YES; break; case AimbotMode_Scope: aim = YES; break; default: break; }
    if (!aim) return;
    ...
    if (dyaw > 180.0f) dyaw -= 360.0f; if (dyaw < -180.0f) dyaw += 360.0f;
    
    // محاولة الكتابة المباشرة (قد تحتاج Hook لتعمل في بعض الإصدارات)
...
- (instancetype)initWithFrame:(CGRect)f {
    ...
    if (self) {
        ...
        [_playerCountLabel setText:@"Players: 0"]; [_playerCountLabel setTextColor:[UIColor colorWithRed:0.0 green:1.0 blue:0.0 alpha:0.9]]; [_playerCountLabel setFont:[UIFont boldSystemFontOfSize:15]]; [_playerCountLabel setTextAlignment:NSTextAlignmentCenter];
        [self addSubview:_playerCountLabel];
        _watermark = [[UILabel alloc] initWithFrame:CGRectMake(-200, f.size.height * 0.92, 220, 25)];
        [_watermark setText:@"RavFen Shadow"]; [_watermark setTextColor:[UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.5]]; [_watermark setFont:[UIFont boldSystemFontOfSize:16]];
...
- (instancetype)initWithFrame:(CGRect)f { self = [super initWithFrame:f]; if (self) { self.backgroundColor = [UIColor blackColor]; UILabel *l = [[UILabel alloc] init]; [l setText:@"R"]; [l setTextColor:[UIColor colorWithRed:1.0 green:0.84 blue:0.0 alpha:1.0]]; [l setFont:[UIFont boldSystemFontOfSize:100]]; [l setTextAlignment:NSTextAlignmentCenter]; l.frame = CGRectMake(0, f.size.height * 0.3, f.size.width, 120); [self addSubview:l]; UILabel *t = [[UILabel alloc] init]; [t setText:@"RavFen Shadow"]; [t setTextColor:[UIColor colorWithRed:1.0 green:0.84 blue:0.0 alpha:0.8]]; [t setFont:[UIFont boldSystemFontOfSize:22]]; [t setTextAlignment:NSTextAlignmentCenter]; t.frame = CGRectMake(0, f.size.height * 0.5, f.size.width, 40); [self addSubview:t]; l.alpha = 0; t.alpha = 0; [UIView animateWithDuration:0.6 delay:0.2 options:0 animations:^{ l.alpha = 1; } completion:nil]; [UIView animateWithDuration:0.5 delay:0.5 options:0 animations:^{ t.alpha = 1; } completion:nil]; } return self; }
@end
...
- (instancetype)initWithFrame:(CGRect)f { self = [super initWithFrame:f]; if (self) { self.backgroundColor = [UIColor blackColor]; UILabel *l = [[UILabel alloc] init]; [l setText:@"Tap to Start"]; [l setTextColor:[UIColor whiteColor]]; [l setFont:[UIFont boldSystemFontOfSize:26]]; [l setTextAlignment:NSTextAlignmentCenter]; l.frame = CGRectMake(0, f.size.height * 0.45, f.size.width, 40); [self addSubview:l]; [UIView animateWithDuration:1.0 delay:0 options:UIViewAnimationOptionAutoreverse | UIViewAnimationOptionRepeat animations:^{ l.alpha = 0.3; } completion:nil]; UITapGestureRecognizer *tg = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tap)]; [self addGestureRecognizer:tg]; } return self; }
...
// 🔘 Floating Button
// ====================================================================
@interface RavFloatingButton : UIView { UILabel *_l, *_rt; UIPanGestureRecognizer *_p; CGFloat _a; }
@property (nonatomic, copy) void (^onTap)(void);
@end
@implementation RavFloatingButton
- (instancetype)initWithFrame:(CGRect)f { self = [super initWithFrame:f]; if (self) { _a = 0; self.backgroundColor = [UIColor colorWithRed:0.3 green:0.0 blue:0.5 alpha:0.75]; self.layer.cornerRadius = f.size.width / 2; self.layer.borderWidth = 2; self.layer.borderColor = [UIColor colorWithRed:1.0 green:0.84 blue:0.0 alpha:0.8].CGColor; _l = [[UILabel alloc] init]; [_l setText:@"R"]; [_l setTextColor:[UIColor colorWithRed:1.0 green:0.84 blue:0.0 alpha:1.0]]; [_l setFont:[UIFont boldSystemFontOfSize:26]]; [_l setTextAlignment:NSTextAlignmentCenter]; _l.frame = self.bounds; _l.userInteractionEnabled = NO; [self addSubview:_l]; _rt = [[UILabel alloc] init]; [_rt setText:@"RavFen"]; [_rt setTextColor:[UIColor colorWithRed:1.0 green:0.84 blue:0.0 alpha:0.5]]; [_rt setFont:[UIFont boldSystemFontOfSize:9]]; _rt.frame = CGRectMake(0, 0, 45, 12); _rt.userInteractionEnabled = NO; [self addSubview:_rt]; _p = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(pan:)]; [self addGestureRecognizer:_p]; UITapGestureRecognizer *tp = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tap)]; [self addGestureRecognizer:tp]; [_p requireGestureRecognizerToFail:tp]; [NSTimer scheduledTimerWithTimeInterval:0.03 repeats:YES block:^(NSTimer *t) { if (!self.superview) { [t invalidate]; return; } _a += 0.1; CGFloat r = 30, cx = self.bounds.size.width / 2, cy = self.bounds.size.height / 2; _rt.frame = CGRectMake(cx + cos(_a) * r - 22, cy + sin(_a) * r - 6, 45, 12); }]; } return self; }
...
// ====================================================================
// 📋 Menu View - بتصميم هيبة وألوان جذابة
// ====================================================================
@interface RavMenuView : UIView { dispatch_queue_t _q; NSTimer *_t; UISlider *_ss, *_ds; UILabel *_sv, *_dv; UISwitch *_as, *_es, *_ls, *_bls; UISegmentedControl *_ms; UIButton *_fps; BOOL _fc; ESPOverlayView *_ev; UIPanGestureRecognizer *_pan; CGPoint _startCenter; }
@end
@implementation RavMenuView
- (instancetype)initWithFrame:(CGRect)f overlayView:(ESPOverlayView *)ov { self = [super initWithFrame:f]; if (self) { _q = dispatch_queue_create("rf.bg", DISPATCH_QUEUE_SERIAL); _ev = ov; self.backgroundColor = [UIColor colorWithRed:0.02 green:0.02 blue:0.06 alpha:0.97]; self.layer.cornerRadius = 12; self.layer.borderWidth = 1; self.layer.borderColor = [UIColor colorWithRed:1.0 green:0.84 blue:0.0 alpha:0.4].CGColor; self.clipsToBounds = YES; _pan = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panMenu:)]; [self addGestureRecognizer:_pan]; [self ui]; [self syncUI]; [self loop]; } return self; }
- (void)panMenu:(UIPanGestureRecognizer *)g { UIView *v = self.superview; if (!v) return; if (g.state == UIGestureRecognizerStateBegan) { _startCenter = self.center; } else if (g.state == UIGestureRecognizerStateChanged) { CGPoint t = [g translationInView:v]; self.center = CGPointMake(_startCenter.x + t.x, _startCenter.y + t.y); } }
- (void)syncUI { pthread_mutex_lock(&g_ConfigMutex); _as.on = gConfig.aimbotEnabled; _ss.value = gConfig.aimbotSpeed; _ms.selectedSegmentIndex = (NSInteger)(gConfig.aimbotMode - 1); _es.on = gConfig.espEnabled; _ls.on = gConfig.espLine; _bls.on = gConfig.espBulletLine; _ds.value = gConfig.espDistance; _fc = gConfig.fps120Enabled; pthread_mutex_unlock(&g_ConfigMutex); [_sv setText:[NSString stringWithFormat:@"%.0f", gConfig.aimbotSpeed]]; [_dv setText:[NSString stringWithFormat:@"%.0fm", gConfig.espDistance]]; if (_fc) { [_fps setTitle:@"✓" forState:UIControlStateNormal]; [_fps setTitleColor:[UIColor greenColor] forState:UIControlStateNormal]; _fps.backgroundColor = [UIColor colorWithRed:0.1 green:0.3 blue:0.1 alpha:0.9]; } else { [_fps setTitle:@"" forState:UIControlStateNormal]; _fps.backgroundColor = [UIColor colorWithRed:0.15 green:0.15 blue:0.15 alpha:0.8]; } }
- (void)ui {
...
- (void)loop { __weak typeof(self) ws = self; _t = [NSTimer scheduledTimerWithTimeInterval:(0.014 + (arc4random() % 4) * 0.001) repeats:YES block:^(NSTimer *t) { typeof(self) ss = ws; if (!ss) { [t invalidate]; return; } dispatch_async(ss->_q, ^{ UpdateLobbyStatus(); NSArray *p = GetPlayers(); DoAimbot(p); // ✅ الإصلاح: إرسال تحديث الواجهة إلى الخيط الرئيسي dispatch_async(dispatch_get_main_queue(), ^{ [ss->_ev updateWithPlayers:p]; }); }); }]; [[NSRunLoop mainRunLoop] addTimer:_t forMode:NSRunLoopCommonModes]; }
